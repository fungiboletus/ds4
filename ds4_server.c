...
static void chat_prepare_live_continuation(server *s, request *r,
                                           const chat_msgs *msgs) {
...
static bool parse_chat_request(ds4_engine *e, server *s, const char *body, int def_tokens,
    ...
    chat_prepare_live_continuation(s, r, &msgs);
...
static void remember_thinking_checkpoint(server *s, server_slot *slot,
    ...
    pthread_mutex_lock(&s->tool_mu);
    slot->thinking_live.token_text_disk_key =
        j->req.prompt_preserves_reasoning && !visible_continuation && !stripped;
    pthread_mutex_unlock(&s->tool_mu);
    server_log(DS4_LOG_KVCACHE,
...
decode_again:
    ...
    if (parsed_calls.len &&
        !should_canonicalize_tool_checkpoint(s, &parsed_calls) &&
        remember_qwen_tool_turn_visible_checkpoint(
            s, slot, j, ctx_span, finish, thinking.inside,
            parsed_content ? parsed_content : "", &parsed_calls)) {
        /* Qwen stops before the assistant end token; preserve its frontier. */
...
static void test_chat_live_tail_renders_tool_results_only(void) {
    ...
    chat_prepare_live_continuation(NULL, &r, &msgs);
    TEST_ASSERT(r.anthropic_live_call_ids.len == 1);
    ...
    chat_prepare_live_continuation(NULL, &r2, &msgs);
...
static void test_deepseek41_live_result_order(void) {
    ...
    for (int protocol = 0; protocol < 3; protocol++) {
        const bool anthropic = protocol == 1;
        const bool openai = protocol == 2;
        request r;
        request_init(&r, REQ_CHAT, 128);
        r.model_syntax = SERVER_MODEL_SYNTAX_DEEPSEEK41;
        r.api = anthropic ? API_ANTHROPIC : openai ? API_OPENAI : API_RESPONSES;
        r.think_mode = DS4_THINK_HIGH;
        live_tool_state *live = (anthropic || openai) ? &slot.anthropic_live : &slot.responses_live;
        live->valid = true;
        ...
        else if (openai) chat_prepare_live_continuation(&s, &r, &msgs);
        else responses_prepare_live_continuation(&s, &r, &msgs);
        const char *tail = (anthropic || openai) ? r.anthropic_live_suffix_text : r.responses_live_suffix_text;
